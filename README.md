...
Этот репозиторий использует автоматическую систему проверки и обновления VPN конфигураций на основе Python скрипта.

### Особенности системы:

- **Мультипоточность:** До 150 параллельных потоков для быстрой проверки
- **Умное кэширование:** IP адреса и Geo данные сохраняются на 30 дней
- **GeoIP фильтрация:** Автоматическое определение страны выхода
- **Ping тестирование:** Отсев медленных серверов (>2000мс)
- **Протокол детектор:** Автоматическое разделение по VLESS/VMess/Trojan/SS
- **Rate limiting:** Соблюдение лимитов API запросов (50 запросов в минуту)
...
├── black/              # Чёрные списки
│   └── black.txt
├── black_mobile/       # Мобильные чёрные списки
│   └── black_mobile.txt
├── white/              # Белые списки
│   ├── white.all.txt
│   ├── white.sni.txt
│   └── white.cidr.txt
├── tg_proxy/           # Telegram прокси
│   └── tg_proxy.txt
...
[⬆️ Вернуться к началу](#-stintikvpn---интернет-свобода-ближе-чем-кажется)
